...
The **profile detect** command will create a profile from the analysis of a \
        MPI binary's execution.
This process uses system call interception to produce an exhaustive list of \
        files required by the MPI process.
...
.. warning::
    ...
    * The process should be run on multiple nodes using the target network. \
            Failure to do so may result in erroneous detection of \
            communication libraries and thus may create communication errors \
