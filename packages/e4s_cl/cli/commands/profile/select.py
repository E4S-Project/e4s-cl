...
class ProfileSelectCommand(AbstractCommand):
    ...
    def _construct_parser(self):
        ...
        parser.add_argument('name', help="Profile name", metavar='<profile_name>')
        return parser
    ...
    def main(self, argv):
        ...
        if not profile:
            self.parser.error("There is no profile configuration named '%s.'" % name)
        profile_ctrl.select(profile)
        return EXIT_SUCCESS


COMMAND = ProfileSelectCommand(__name__, summary_fmt=("Select a profile.\n"
